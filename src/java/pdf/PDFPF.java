 ...
 * @author Kevin
 */
public class PDFPF extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            response.setHeader("Content-Disposition", "inline; filename=Clientes fiadores");
            ...
            par1 = new Paragraph(new Phrase("REPORTE DE CLIENTES FIADORES", fonttitulo));
            par1.setAlignment(Element.ALIGN_CENTER);
            par1.add(new Phrase(Chunk.NEWLINE));//salto de linea
            par1.add(new Phrase(Chunk.NEWLINE));//salto de linea
            par1.add(new Phrase(Chunk.NEWLINE));//salto de linea
            ...
            //emcabezado
            cell = new PdfPCell(new Paragraph("TABLA DE INFORMACIÓN SOBRE LOS CLIENTES NATURALES QUE HAN OBTENIDO UN PRESTAMO", FontFactory.getFont("courier", 12, Font.BOLD, BaseColor.BLACK)));
            ...
            for (Cnatural c : cn) {
                ...
                cell = new PdfPCell(new Paragraph(c.getPersona().getDireccion(), FontFactory.getFont("courier", 12, Font.NORMAL, BaseColor.BLACK)));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setRowspan(2);
                tabla.addCell(cell);

                cell = new PdfPCell(new Paragraph(c.getPersona().getEstadocivil(), FontFactory.getFont("courier", 12, Font.NORMAL, BaseColor.BLACK)));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setRowspan(2);
                tabla.addCell(cell);

                cell = new PdfPCell(new Paragraph(c.getLugartrabajo(), FontFactory.getFont("courier", 12, Font.NORMAL, BaseColor.BLACK)));
                cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                cell.setRowspan(2);
                tabla.addCell(cell);
                
                for (Disponibilidad d : c.getDisponibilidadsForIdfiador()) {
            ...
            Logger.getLogger(PDFPF.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
