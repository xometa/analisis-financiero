 ...
 * @author BONIFACIO
 */
public class ServletPersonaNatural extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!accion.isEmpty() && accion != null) {
                if (accion.equals("eliminar")) {
                    ...
                    cn = mf.recuperaPersonaNatural(id);
                    if (cn.getDisponibilidadsForIdfiador().size() > 0) {
                        json = muestramensaje("Eliminar Cliente Natural", "El cliente natural no se debe eliminar, ya que posee ingresos o egresos.", "success", null);
                        out.print(json);
                        return;
                    }
                    if (mf.eliminarPersonaNatural(cn) == 1) {
                        if (mf.eliminarPersonaNatural(cn.getPersona()) == 1) {
                            json = muestramensaje("Eliminar Cliente Natural", "El cliente natural se ha eliminado correctamente.", "success", cn);
                            out.print(json);
                        } else {
                            json = muestramensaje("Eliminar Cliente Natural", "El cliente natural no se ha eliminado.", "success", null);
                            out.print(json);
                        }
                    } else {
                        json = muestramensaje("Eliminar Cliente Natural", "El cliente natural no se ha eliminado.", "success", null);
                    ...
                    if (accion.equals("guardar")) {
                        if (mf.recuperaPersona(0, dui, "p.dui=?") == null) {
                            if (mf.guardarPersonaNatural(persona, cn) == 1) {
                                json = muestramensaje("Registrar Cliente Natural", "El cliente natural se ha registrado correctamente.", "success", cn);
                                out.print(json);
                            } else {
                                json = muestramensaje("Registrar Cliente Natural", "El cliente natural no se ha registrado.", "error", null);
                                out.print(json);
                            }
                        } else {
                            json = muestramensaje("Registrar Cliente Natural", "El número de DUI ingresado para el cliente natural ya existe.", "info", null);
                            out.print(json);
                        }
                    } else {
                        int id = Integer.parseInt(request.getParameter("id"));
                        ...
                        if (!aux.getDui().equals(dui)) {
                            ...
                            if (aux != null) {
                                if (aux.getDui().equals(dui)) {
                                    json = muestramensaje("Modificar Cliente Natural", "El número de DUI ingresado ya existe.", "info", null);
                                    out.print(json);
                                    return;
                                }
                            }
                        ...
                        if (mf.recuperaPersonaNatural(id) != null) {
                            if (mf.modificarPersonaNatural(persona, cn) == 1) {
                                json = muestramensaje("Modificar Cliente Natural", "El cliente natural se ha modificado correctamente.", "success", cn);
                                out.print(json);
                            } else {
                                json = muestramensaje("Modificar Cliente Natural", "El cliente natural no se ha modificado.", "error", null);
                                out.print(json);
                            }
                        } else {
                    ...
                    if (mf.eliminarPersonaNatural(disponibilidad) == 1) {
                    ...
                    cn = mf.recuperaPersonaNatural(idfiador);
                    disponibilidad = new Disponibilidad(cn, null, descripcion, monto, tipo);
