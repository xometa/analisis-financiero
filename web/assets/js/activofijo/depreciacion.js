...
let contenedor = query.id('listado-activos'),
        infoDepreciacion = query.id('depreciacion-activofijo'),
        plantilla = query.id('vacio'),
        idactivo = 0;

infoDepreciacion.innerHTML = plantilla.innerHTML;
...
function obtenerDepreciacion(id) {
    query.get({
        url: "ajax/activofijo/infodepreciacion.jsp",
        method: 'get',
        data: {id: id},
        success: function (data) {
            infoDepreciacion.innerHTML = data;
        },
        fail: function () {
            query.toast("Error", "La depreciación del activo fijo no se puede visualizar.", "error");
        }
    });
}
...
contenedor.onclick = function (e) {
    ...
    if (current.matches('td')) {
        ...
        if (!query.validate(id, "int") && !query.validate(id, "numbers")) {
            obtenerDepreciacion(parseInt(id))
        } else {
