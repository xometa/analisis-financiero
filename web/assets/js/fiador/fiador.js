...
function listadoFiadores() {
    query.get({
        url: "ajax/fiador/listadofiadores.jsp",
        method: 'get',
...
function listadoDiferencia(data) {
    query.get({
        url: "ajax/fiador/diferencia.jsp",
        method: 'get',
        data: data,
        success: function (data) {
...
function recuperarCuerpoModal(id) {
    ...
    query.get({
        url: "ajax/fiador/modalfiador.jsp",
        method: 'get',
        data: data,
        success: function (data) {
...
function iconoModalBoton(tittle) {
    if (tittle.textContent == "Nuevo Fiador") {
    ...
    } else if (tittle.textContent == "Modificar Fiador") {
...
buttonModalClient.onclick = (e) => {
    modalTitle.innerHTML = "Nuevo Fiador";
...
contenedor.onclick = function (e) {
    ...
    if (actual.matches('i[id=editar]') || actual.classList.contains('btn-warning')) {
        ...
        Id = actual.getAttribute("data-idfiador");
        if (!query.validate(Id, "int") && !query.validate(Id, "numbers")) {
            tituloEliminar.innerHTML = "Eliminar fiador";
...
function acciones(data) {
    if (!query.validate(data.accion, "string")) {
        if (data.accion == "editar") {
            modalTitle.innerHTML = "Modificar Fiador";
            iconoModalBoton(modalTitle);
...
formularioFiador.onsubmit = function (e) {
    ...
    if (query.validate(query.id('codigo').value, "string")) {
        query.toast("Campos vacíos", "Ingrese el código del fiador", "warning");
        return;
    }

    if (query.validate(query.id('dui').value, "string")) {
        query.toast("Campos vacíos", "Ingrese el DUI del fiador", "warning");
        return;
    }

    if (query.validate(query.id('nombre').value, "string")) {
        query.toast("Campos vacíos", "Ingrese el nombre del fiador", "warning");
        return;
    }

    if (query.validate(query.id('apellido').value, "string")) {
        query.toast("Campos vacíos", "Ingrese el apellido del fiador", "warning");
        return;
    }

    if (!query.id('masculino').checked && !query.id('femenino').checked) {
        query.toast("Campos vacíos", "Seleccione el sexo del fiador", "warning");
        return;
    }

    if (query.validate(query.id('fechanacimiento').value, "string")) {
        query.toast("Campos vacíos", "Ingrese la fecha de nacimiento del fiador", "warning");
        return;
    }

    if (query.validate(query.id('telefono').value, "string")) {
        query.toast("Campos vacíos", "Ingrese el # de teléfono del fiador", "warning");
        return;
    }

    if (query.validate(query.id('direccion').value, "string")) {
        query.toast("Campos vacíos", "Ingrese la dirección del fiador", "warning");
        return;
    }

    if (query.validate(query.id('estadocivil').value, "string")) {
        query.toast("Campos vacíos", "Seleccione el estado civil del fiador", "warning");
        return;
    }

    if (query.validate(query.id('lugartrabajo').value, "string")) {
        query.toast("Campos vacíos", "Ingrese el lugar de trabajo del fiador", "warning");
        return;
    }
    ...
    if (!query.validate(accion, "string")) {
        ...
        query.ajax({
            ...
            success: function (data) {
                ...
                query.toast("Registrar fiador", texto, icono);
                console.log(objeto);
                if (objeto != "null") {
                    listadoFiadores("ajax/fiador/listadofiadores.jsp", null, contenedor);
                    $("#modal-fiador").modal("hide");
                }
            },
            fail: function () {
